...
// https://ImageToolLearn.io/docs/golden-retriever/
...
self.addEventListener('install', (event) => {
  console.log('Installing ImageToolLearn Service Worker...')
...
self.addEventListener('message', (event) => {
  switch (event.data.type) {
    case 'ImageToolLearn/ADD_FILE':
      addFile(event.data.store, event.data.file)
      break
    case 'ImageToolLearn/REMOVE_FILE':
      removeFile(event.data.store, event.data.fileID)
      break
    case 'ImageToolLearn/GET_FILES':
