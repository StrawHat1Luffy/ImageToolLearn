...
describe('dashboard-compressor', () => {
  ...
  it('should compress images', () => {
    ...
    cy.get('.ImageToolLearn-Dashboard-Item-statusSize').each((element) => {
      const text = element.text()
      sizeBeforeCompression.push(uglierBytes(text))
    })

    cy.window().then(({ ImageToolLearn }) => {
      ImageToolLearn.on('preprocess-complete', (file) => {
        expect(file.extension).to.equal('webp')
        expect(file.type).to.equal('image/webp')

        cy.get('.ImageToolLearn-Dashboard-Item-statusSize').should((elements) => {
