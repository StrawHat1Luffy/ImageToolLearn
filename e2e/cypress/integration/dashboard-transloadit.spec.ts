describe('Dashboard with Transloadit', () => {
    ...
    cy.window().then(({ ImageToolLearn }) => {
      cy.get('@file-input').selectFile(
        [
          ...
          'cypress/fixtures/images/carToCheck.jpg',
        ],
        { force: true },
      )
      cy.get('.ImageToolLearn-StatusBar-actionBtn--upload').click()
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(ImageToolLearn.getPlugin('Transloadit').activeAssemblies).every(
            (a: any) => a.pollInterval,
          ),
        ).to.equal(true)

        ImageToolLearn.cancelAll()

        cy.wait(['@delete']).then(() => {
          // eslint-disable-next-line
          // @ts-ignore fix me
          expect(
            Object.values(ImageToolLearn.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ ImageToolLearn }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      ImageToolLearn.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ ImageToolLearn }) => {
      ...
      const { files } = ImageToolLearn.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      ImageToolLearn.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ ImageToolLearn }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(ImageToolLearn.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it.skip('should create assembly if there is still one file to upload', () => {
    ...
    cy.window().then(({ ImageToolLearn }) => {
      ...
      ImageToolLearn.removeFile(fileID)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ ImageToolLearn }) => {
      const { files } = ImageToolLearn.getState()
      ImageToolLearn.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        ImageToolLearnserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'ImageToolLearn-core:3.2.0,ImageToolLearn-transloadit:3.1.3,ImageToolLearn-tus:3.1.0,ImageToolLearn-dropbox:3.1.1,ImageToolLearn-box:2.1.1,ImageToolLearn-facebook:3.1.1,ImageToolLearn-google-drive:3.1.1,ImageToolLearn-instagram:3.1.1,ImageToolLearn-onedrive:3.1.1,ImageToolLearn-zoom:2.1.1,ImageToolLearn-url:3.3.1',
    ...
    cy.get('.ImageToolLearn-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.ImageToolLearn-StatusBar-actions > .ImageToolLearn-c-btn').click()
    cy.wait([
